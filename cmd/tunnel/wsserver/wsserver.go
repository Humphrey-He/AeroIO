// Package wsserver provides a WebSocket-based tunnel server gateway.
package wsserver
...
// SetOnPortOpen sets callback for port open events
func (s *Server) SetOnPortOpen(fn func(*PortBinding)) { s.onPortOpen = fn }

// SetOnPortClose sets callback for port close events
func (s *Server) SetOnPortClose(fn func(*PortBinding)) { s.onPortClose = fn }

// ListenAndServe starts the WebSocket tunnel server
func (s *Server) ListenAndServe() error {
	...
	log.Printf("[WSServer] WebSocket server listening on %s", s.config.ListenAddr)
	...
	for s.running.Load() {
		...
		if err != nil {
			...
			log.Printf("[WSServer] Accept error: %v", err)
			continue
		}
...
func (s *Server) Stop() {
	...
	log.Printf("[WSServer] WebSocket server stopped")
}
...
func (s *Server) handleWSSession(conn net.Conn) {
		...
		for {
			...
			case 0x2: // Binary
				...
				if msg.Type == tunnel.MsgRegister {
					...
					log.Printf("[WSServer] Client connected: %s", sessionID)
				} else if session != nil {
	...
	<-stopCh
	if session != nil {
		log.Printf("[WSServer] Client disconnected: %s", session.ID)
	}
	conn.Close()
}
...
func (s *Server) handleWSMessage(session *Session, msg *tunnel.Message, conn net.Conn) {
	...
	case tunnel.MsgOpenPort:
		...
		log.Printf("[WSServer] Port opened: :%d -> %s (channel: %d)", publicPort, targetAddr, ch.ID)
